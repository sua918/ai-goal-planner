"""캘린더 UI와 ICS 내보내기. 계획 생성과 검증은 core.py가 담당한다."""
...
from core import ScheduleResult, generate_schedule, is_mock_mode
...
EXAMPLE_INPUT = """10월 중순 한국사 시험이 있고 SKCT도 준비해야 해.
...
# %% app_styles
# 화면 CSS와 캘린더 iframe CSS는 적용 대상이 달라 따로 보관한다.
APP_CSS = f"""
<style>
...
def _fold_ics_line(line: str) -> str:
    """한글은 여러 바이트이므로 글자 수가 아닌 UTF-8 75바이트 기준으로 접는다."""
    parts, current, size = [], "", 0
    for character in line:
        width = len(character.encode("utf-8"))
        if size + width > 75:
            parts.append(current)
            current, size = " ", 1  # 이어지는 행의 첫 공백도 바이트 길이에 포함한다.
        current += character
        size += width
    parts.append(current)
    return "\r\n".join(parts)
...
def _schedule_to_ics(result: ScheduleResult) -> bytes:
    """현재 결과 전체를 파일로 만든다. 다운로드 때문에 LLM을 다시 호출하지 않는다."""
    seoul = ZoneInfo("Asia/Seoul")
    created_at = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    lines = [
        "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//AI Goal Planner//Schedule//KO",
        "CALSCALE:GREGORIAN", "METHOD:PUBLISH", "X-WR-CALNAME:AI 목표 플래너",
    ]
    for event in result.events:
        # 기존 일정의 naive datetime은 한국 시각으로 해석하고 UTC로 내보낸다.
...
def _strategy_items(summary: str | None) -> list[str]:
    """전략 문자열을 최대 네 항목으로 나눈다. 시간 범위 표기는 그대로 유지한다."""
    if not summary:
        return []
    chunks = re.split(r"\n+|(?<=[.!?])\s+", summary.strip())
...
def _calendar_events(result: ScheduleResult) -> list[dict]:
    """일정 시간은 바꾸지 않고 색·축약 표시용 클래스만 지정한다."""
    events = []
    for event in result.events:
...
def _plan_dialog() -> None:
    """편집 중에는 기존 결과를 유지하고, 생성에 성공한 경우에만 교체한다."""
...
def _render_goal_chips(result: ScheduleResult) -> None:
    """원문과 LLM 출력은 escape한 뒤 HTML에 넣어 태그로 해석되지 않게 한다."""
    chips = []
...
def _render_list(items: list[str], class_name: str = "detail-list") -> None:
    if items:
        content = "".join(f"<li>{escape(item)}</li>" for item in items)
        # st.html을 써서 물결표·별표가 Markdown 문법으로 변하는 것을 막는다.
        st.html(f'<ul class="{class_name}">{content}</ul>')


# %% ui_details
def _render_plan_details(result: ScheduleResult) -> None:
    """계획 근거·사용자 조건·해석 가정을 보조 영역에 표시한다."""
    with st.expander("AI가 이해한 계획"):
        if result.strategy_summary:
            st.caption("계획 전략")
            _render_list(_strategy_items(result.strategy_summary), "strategy-list")
    ...
    with st.expander("가정 및 주의사항"):
        ...
        if result.plan_spec.warnings:
            ...
            _render_list(result.plan_spec.warnings)
        if result.plan_spec.constraints:
            st.caption("기타 사용자 조건")
            _render_list(result.plan_spec.constraints)

# %% ui_header
def _render_header(result: ScheduleResult | None) -> None:
    """계획 정보와 입력·다운로드 버튼만 상단에 배치한다."""
    ...
    with header_action:
        ...
        if st.button(button_label, type="primary", use_container_width=True):
            st.dialog("새 계획 만들기", width="large")(_plan_dialog)()
    if result:
        with columns[2]:
            st.download_button(
                "캘린더 다운로드",
...
# %% ui_calendar
def _calendar_options(result: ScheduleResult) -> dict:
    """화면 설정만 반환한다. 실제 일정 데이터나 배치는 수정하지 않는다."""
    return {
...
def _render_calendar(result: ScheduleResult) -> None:
    """캘린더와 전체 제목 표시 영역을 그린다."""
    with st.container(border=True):
        ...
        calendar_options = _calendar_options(result)
        ...
        state = calendar_state or {}
        event_payload = (state.get("eventMouseEnter", {}).get("event")
                         or state.get("eventClick", {}).get("event"))
        if event_payload:
...
# %% ui_entry_point
def render_app() -> None:
    """Streamlit 진입점. 이 파일을 import하는 것만으로 UI를 실행하지 않는다."""
